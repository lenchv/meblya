...
<?$APPLICATION->AddHeadScript(SITE_TEMPLATE_PATH."/components/bitrix/system.auth.form/meblya_auth_form/script.js");?>
<?$APPLICATION->SetAdditionalCSS(SITE_TEMPLATE_PATH."/components/bitrix/system.auth.form/meblya_auth_form/style.css");?>
...
if (count($arResult["ERRORS"]) > 0):
	...
	foreach ($arResult["ERRORS"] as $key => $error)
		if (intval($key) == 0 ) 
		{
			echo "<div class='my-auth-form-error-message' style='top: ".$ofs."px'>";
			echo "<div class='ic-error'></div>";
//$arResult["ERRORS"][$key] =
...
<?endif?>
	...
	<?if(!$USER->IsAuthorized()):?>
		<a class="registration-link">Register</a>
		<div class="modal-wrapper">
		<div class="bx-auth-reg my-reg-form">
		<form method="post" action="<?=POST_FORM_ACTION_URI?>" name="regform" enctype="multipart/form-data">
			...
			<table>
				<thead>
					<tr>
						<td colspan="2" class="my-register-form-title"><b><?=GetMessage("AUTH_REGISTER")?></b></td>
					</tr>
				</thead>
				<tbody>
			<?foreach ($arResult["SHOW_FIELDS"] as $FIELD):?>
				...
				<?else:?>
					<tr>
						<td class="my-register-form-sign"><?=GetMessage("REGISTER_FIELD_".$FIELD)?>:<?if ($arResult["REQUIRED_FIELDS_FLAGS"][$FIELD] == "Y"):?><span class="starrequired">*</span><?endif?></td>
						<td><?
				switch ($FIELD)
				{
					case "PASSWORD":
			...
			?>
				...
				<tfoot>
					<tr>
						<td colspan="2"><input type="submit" name="register_submit_button" value="<?=GetMessage("AUTH_REGISTER")?>" /></td>
					</tr>
				</tfoot>
			</table>
